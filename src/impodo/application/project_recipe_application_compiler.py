...
class ProjectRecipeApplicationAssessment:
...
@dataclass(frozen=True, slots=True)
class ProjectRecipeMaterialization:
...
class ProjectRecipeApplicationCompiler(RecipeApplicationService):
    """Reuse the proven Recipe drift and mapping compiler without legacy roots.

    The M4 adapter deliberately does not call ``RecipeApplicationService``
    creation methods.  It supplies one already-provisioned clean workspace,
    one immutable Project Recipe envelope, one DataVersion source projection,
    and one run-level target projection to the retained compiler helpers.
    """
