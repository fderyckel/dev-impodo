...
class ProjectRecipeRepository:
    ...
    def read_recipe_revision(
        ...
        try:
            ...
            raise ProjectRecipeError("Stored Recipe envelope is invalid") from error
        if not isinstance(envelope, dict):
            raise ProjectRecipeError("Stored Recipe envelope is invalid")
        claimed = envelope.pop("payload_hash", None)
        if claimed != revision.payload_hash or content_hash(envelope) != claimed:
            raise ProjectRecipeError("Stored Recipe payload hash is inconsistent")
        envelope["payload_hash"] = claimed
        if envelope.get("semantic_hash") != revision.semantic_hash:
            raise ProjectRecipeError("Stored Recipe semantic hash is inconsistent")
        return envelope
    ...
    @staticmethod
    def _recipe_from_row(row: Mapping[str, object]) -> ProjectRecipe:
        current = row.get("current_recipe_revision")
        if current is None:
            raise ProjectRecipeError("Stored Recipe has no published revision")
    ...
    @staticmethod
    def _revision_from_row(row: Mapping[str, object]) -> ProjectRecipeRevision:
        return ProjectRecipeRevision(
            recipe_id=str(row["recipe_id"]),
