# Install Impodo on a Windows laptop

## Purpose

This guide starts at the point where the Impodo source code has been
downloaded from GitHub. It explains every required step to install and start
the local browser application on a Windows laptop.
...
described in [Install an accepted internal bundle](#install-an-accepted-internal-bundle).
...
[local Odoo runbook](02-local-odoo.md). Impodo does not install Odoo or
...
## Install an accepted internal bundle

This is the preferred installation route for an approved internal pilot with
confidential data. An accepted bundle is not the same as the GitHub source
folder. The bundle contains, at minimum:

- `release-manifest.json`;
- one `impodo-*.whl`;
- `requirements.windows-py312.lock`;
- test, secret-scan, dependency-audit, and SBOM evidence; and
- `install-internal-release.ps1`.

Keep the bundle unchanged in an approved local directory. Open PowerShell in
that bundle and run:
...
The installer verifies the manifest, file sizes, SHA-256 hashes, evidence,
Python 3.12 runtime, locked binary dependencies, Impodo wheel, and installed
dependency consistency. It then prints the exact versioned launcher below:

```text
%LOCALAPPDATA%\Impodo\app\<release-id>\Scripts\impodo.exe
```

Use that printed path to start the accepted release. Do not run
`scripts\install-internal-release.ps1` directly from a GitHub checkout: the
source tree is not a generated release bundle and does not contain the
required release manifest and wheel.

The current bundle still downloads locked third-party wheels from the approved
package source; it is not a fully offline or digitally signed enterprise
installer. See the [internal release runbook](06-internal-release.md) for the
promotion and acceptance process.
