...
| Local Match data work order | `domain/matching_order.py` owns pure component ordering, typed facts, and the isolated preference contract; `application/workspace/mapping/order_service.py` derives advice and reconciles custom order; `adapters/duckdb/matching_order_repository.py` stores only the versioned workspace preference; `web/presenters/mapping_view.py`, `web/templates/mapping/_matching_order.html`, and `web/static/mapping-order.js` project and rearrange the queue without changing stable form indexes, semantic evidence, or Odoo |
