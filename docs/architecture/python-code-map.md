...
| Database boundary | [`adapters/duckdb/database.py`](../../src/impodo/adapters/duckdb/database.py) and [`unit_of_work.py`](../../src/impodo/adapters/duckdb/unit_of_work.py) | One hardened connection factory disables external access/extensions. Per-project UUID containment, current-schema validation, explicit begin/commit/rollback, and shared unit-of-work scopes sit below all concrete repositories. |
| Development schema | [`adapters/duckdb/schema/project.py`](../../src/impodo/adapters/duckdb/schema/project.py) | New databases receive the complete current schema. Impodo deliberately rejects a different schema version with a delete-and-recreate instruction; development project databases are disposable and no backward schema migrations are maintained. |
