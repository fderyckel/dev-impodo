...
| Seal and review completed-load correction intent | `domain/correction.py`, `domain/correction_origin.py`, `application/correction_service.py`, `application/correction_orchestration.py` | `adapters/polars_correction.py` and `adapters/correction_review_pipeline.py` reuse native prepared Parquet columns; `adapters/duckdb/correction_repository.py` owns one binding/current pointer; `adapters/protected_correction_store.py` encrypts origins, indexes, plans, and confirmations without row hashes |
...
| Completed-load correction decisions | `domain/correction.py`, `domain/correction_origin.py`, `application/correction_service.py`, and `application/correction_orchestration.py` | `adapters/polars_correction.py` emits sparse typed A/C differences from the two immutable prepared Parquet artifacts; `adapters/correction_review_pipeline.py` requires native programs and orders sparse adaptation; `adapters/duckdb/correction_repository.py` atomically seals history, invalidates stale successor evidence on changed mapping intent, and owns current pointers; `adapters/protected_correction_store.py` stores the lean origin, exact-target index, deterministic scalar plan, and separately bound confirmation; relationship candidates remain separately write-qualified |
