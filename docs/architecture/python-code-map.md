...
| Seal, review, execute, and verify completed-load correction intent | `domain/correction.py`, `domain/correction_origin.py`, `domain/correction_execution.py`, `application/correction_service.py`, `application/correction_orchestration.py`, `application/correction_execution.py` | Native Polars/Parquet review emits sparse intent; the correction service uses exact protected IDs, bounded reread and writes, the existing execution/reconciliation journals, and one registry-owned binding; protected evidence remains whole-artifact hashed without row hashes |
...
| Completed-load correction decisions | `domain/correction.py`, `domain/correction_origin.py`, `domain/correction_execution.py`, `application/correction_service.py`, `application/correction_orchestration.py`, and `application/correction_execution.py` | `adapters/polars_correction.py` emits sparse typed A/C differences from immutable prepared Parquet artifacts; `adapters/correction_review_pipeline.py` requires native programs; the correction executor uses exact protected IDs and the existing scoped JSON-2, execution-journal, and reconciliation adapters; `adapters/duckdb/correction_repository.py` owns current pointers and verified successor completion; relationship candidates remain separately write-qualified |
