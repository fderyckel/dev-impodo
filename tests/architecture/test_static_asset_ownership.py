...
class StaticAssetOwnershipTests(unittest.TestCase):
    def test_base_template_loads_only_bounded_shared_assets(self) -> None:
        ...
        self.assertIn("/operation-state.js", template)
        self.assertLess(
    ...
    def test_shared_recovery_ui_uses_bounded_authenticated_heartbeat(self) -> None:
        ...
        self.assertIn("Connection to Impodo was interrupted", template)
        self.assertIn('data-health-url="/health"', template)
        self.assertIn('data-heartbeat-interval-ms="4000"', template)
        self.assertIn('data-heartbeat-timeout-ms="4000"', template)
        self.assertIn('data-delayed-after-ms="20000"', template)
        self.assertIn('data-disconnected-after-ms="45000"', template)
        self.assertIn("data-server-recovery-help", template)
        self.assertIn('action="/diagnostics/bundle"', template)
        self.assertIn("Create diagnostic bundle", template)
        self.assertIn('let connectivity = "CONNECTED"', script)
        self.assertIn('let operation = "IDLE"', script)
        self.assertIn('operation = "DELAYED"', script)
        self.assertIn('operation === "OUTCOME_UNKNOWN"', script)
        self.assertIn('connectivity = "DISCONNECTED"', script)
        self.assertIn('connectivity = "SESSION_ENDED"', script)
        self.assertIn("lastResponseAt = now()", script)
        self.assertIn("now() - lastResponseAt >= disconnectedMs", script)
        self.assertNotIn("consecutiveFailures", script)
        self.assertIn('credentials: "same-origin"', script)
        self.assertIn('cache: "no-store"', script)
        self.assertIn("response?.status === 401", script)
        self.assertIn("This Impodo session has ended", script)
        self.assertIn('new CustomEvent("impodo:server-disconnected")', script)
        self.assertIn('new CustomEvent("impodo:session-ended")', script)
        self.assertIn("noteResponse", script)
        self.assertIn("impodo:operation-started", script)
        self.assertIn('"impodo:server-disconnected"', shared_script)
        self.assertIn('"impodo:session-ended"', shared_script)
        self.assertIn('"impodo:server-disconnected"', mapping_editor)
        self.assertIn('"impodo:session-ended"', mapping_editor)
        self.assertIn("check the save outcome before retrying", mapping_editor)
        job_polling = (STATIC_ROOT / "job-polling.js").read_text(encoding="utf-8")
        operation_state = (STATIC_ROOT / "operation-state.js").read_text(
            encoding="utf-8"
        )
        self.assertIn("noteResponse?.(response)", job_polling)
        self.assertIn("impodo:operation-started", operation_state)
        self.assertNotIn("window.fetch =", script + operation_state)
