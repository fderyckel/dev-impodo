...
from impodo.adapters.sqlite.preparation_job_repository import (
...
class PreparationJobRepositoryTests(unittest.TestCase):
    def setUp(self) -> None:
        (ROOT / ".tmp").mkdir(exist_ok=True)
        self.temporary = tempfile.TemporaryDirectory(dir=ROOT / ".tmp")
        self.repository = PreparationJobRepository(self.temporary.name)
    ...
    def test_one_active_attempt_progress_cancel_and_retry(self) -> None:
        queued, created = self.repository.enqueue(
            self.project_id,
            "Large products",
            100_000,
            LOCAL_ACTOR.identity,
        )
        repeated, repeated_created = self.repository.enqueue(
            self.project_id,
            "Large products",
            100_000,
            LOCAL_ACTOR.identity,
        ...
        running = self.repository.mark_running(queued.job_id)
        self.assertEqual(running.status, PreparationJobStatus.RUNNING)
        progressed = self.repository.update_progress(
            queued.job_id,
        ...
        stopping = self.repository.request_cancel(self.project_id, queued.job_id)
        self.assertTrue(stopping.cancel_requested)
        stopped = self.repository.mark_cancelled(queued.job_id)
        self.assertEqual(stopped.status, PreparationJobStatus.CANCELLED)

        retry, retry_created = self.repository.enqueue(
            self.project_id,
            "Large products",
            100_000,
            LOCAL_ACTOR.identity,
    ...
    def test_terminal_job_cannot_be_reopened_and_interrupted_job_is_recovered(self) -> None:
        queued, _created = self.repository.enqueue(
            self.project_id,
            "Large BOM",
            100_000,
            LOCAL_ACTOR.identity,
        )
        self.repository.mark_running(queued.job_id)

        reopened = PreparationJobRepository(
            self.temporary.name,
            recover_interrupted=True,
        )
        recovered = reopened.get(self.project_id, queued.job_id)
        ...
        with self.assertRaises(PreparationJobStateError):
            reopened.mark_running(queued.job_id)
