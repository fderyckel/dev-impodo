...
class PreparationWorkflowBrowserTests(ProjectSetupBrowserTestCase):
    ...
    def test_prepare_rejects_bad_source_hash_before_publication_and_redirects(
        ...
        self.assertIn("Stopped at", recovery.text)
        self.assertIn("data-preparation-spinner hidden", recovery.text)
        self.assertIn("data-preparation-active hidden", recovery.text)
        self.assertIn("The prepared columnar snapshot could not be verified", recovery.text)
