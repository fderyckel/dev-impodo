    ...
    try:
        ...
        with patch.object(context.run_planning, "target_evidence_from_workspace", return_value=(fixture.schema, None)), \
            ...
            with sync_playwright() as playwright:
                ...
                page.goto(base_url + fixture.fresh_url, wait_until="networkidle")
                details = page.get_by_role("heading", name="Details for this run").locator("..")
                details.scroll_into_view_if_needed()
                page.screenshot(path=str(REPOSITORY_ROOT / "docs/images/user/05c-production-fresh-data.png"), full_page=False)
                page.goto(base_url + fixture.run_url + "/odoo", wait_until="networkidle")
                page.screenshot(path=str(REPOSITORY_ROOT / "docs/images/user/05d-production-odoo-check.png"), full_page=False)
                page.goto(base_url + fixture.url, wait_until="networkidle")
                expect(page.locator('[name="control_0"]')).to_have_count(0)
